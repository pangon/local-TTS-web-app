...
The project is in the Code phase. Architecture drafted (updated 2026-04-11 — added Model-Specific Loading Requirements section with compatibility table and adapter pattern); data model drafted (updated 2026-03-11); API design drafted (updated 2026-04-11 — added `loader_available` field to `GET /models` response). 8 decisions recorded (DEC-fastapi-backend, DEC-vue3-frontend, DEC-sqlite-metadata, DEC-single-process, DEC-sse-progress, DEC-tts-as-backend-module, DEC-python-backend-env, DEC-frontend-dev-env). Objectives: 10 Goals, 12 User Stories, 28 Requirements (all approved); 6 Constraints (all active); 5 Assumptions (all verified). Completeness assessment (2026-03-11): 0 Critical, 0 Important (ASM-huggingface-models-available now verified), 3 Minor (stale — design changed since; refresh recommended). 2 components identified (2026-03-14): frontend, backend; TTS engine merged into backend as subpackage per DEC-tts-as-backend-module. Implementation plan created (2026-03-12, updated 2026-04-11): 8 phases + Phase 3.1, 63 tasks covering all 10 approved goals; Phase 3.1 added for model adapter abstraction and per-model loaders (15 tasks). Implementation progress: 27/63 tasks done, Phase 3 complete (6/6 tasks done), Phase 3.1 in progress (3/15 tasks done — TASK-model-adapter-interface, TASK-model-loader-status, TASK-loader-kokoro complete), Phase 4 in progress (5/6 tasks done — TASK-job-service, TASK-synthesis-job-api, TASK-job-progress-sse, TASK-library-service-create, TASK-audiobook-creation-view complete; TASK-phase-4-manual-testing next).
