...
| TASK-preprocessing-pipeline-skeleton | Create Preprocessing Service: stage interface, pipeline runner, and language/model profile config resolution | P1 | Todo | [REQ-MNT-preprocessing-pipeline](../1-objectives/requirements/REQ-MNT-preprocessing-pipeline.md) | TASK-python-project-scaffold | 2026-06-16 | Phase 5.1. Separate backend service per DEC-text-preprocessing-pipeline; not in the TTS subpackage |
| TASK-stage-unicode-sanitization | Implement Unicode sanitization stage (invisible/control chars, NBSP/whitespace variants, dash/quote variants, disallowed Unicode, emoji remove-or-verbalize) | P1 | Todo | [REQ-F-text-unicode-sanitization](../1-objectives/requirements/REQ-F-text-unicode-sanitization.md) | TASK-preprocessing-pipeline-skeleton | 2026-06-16 | Phase 5.1 |
| TASK-stage-layout-repair | Implement layout-repair stage (end-of-line de-hyphenation, sentence reflow, page-number/fragment stripping, whitespace normalization; preserve paragraph/chapter boundaries) | P1 | Todo | [REQ-F-text-layout-repair](../1-objectives/requirements/REQ-F-text-layout-repair.md) | TASK-preprocessing-pipeline-skeleton | 2026-06-16 | Phase 5.1. Runs before chapter detection; must not defeat REQ-F-chapter-split-output |
| TASK-stage-numeric-symbolic-verbalization | Implement language-aware numeric, date, percentage, currency, and symbol verbalization stage | P1 | Todo | [REQ-F-text-numeric-symbolic-verbalization](../1-objectives/requirements/REQ-F-text-numeric-symbolic-verbalization.md) | TASK-preprocessing-pipeline-skeleton | 2026-06-16 | Phase 5.1. Default rules target Italian (DEC-default-italian-language) |
| TASK-stage-abbreviation-expansion | Implement abbreviation expansion stage with built-in language set and optional on-disk domain dictionary | P2 | Todo | [REQ-F-abbreviation-expansion](../1-objectives/requirements/REQ-F-abbreviation-expansion.md) | TASK-preprocessing-pipeline-skeleton | 2026-06-16 | Phase 5.1. Domain dictionary delivery (e.g. config/preprocessing/domain_dictionary.json) refinable; absence must not break preprocessing |
| TASK-preprocess-api | Implement POST /preprocess (file or text input), apply loaded-model preprocessing profile, return normalized text | P1 | Todo | [REQ-F-upload-text-file](../1-objectives/requirements/REQ-F-upload-text-file.md), [REQ-USA-normalized-text-review](../1-objectives/requirements/REQ-USA-normalized-text-review.md), [REQ-PERF-preprocessing-overhead](../1-objectives/requirements/REQ-PERF-preprocessing-overhead.md) | TASK-preprocessing-pipeline-skeleton, TASK-stage-unicode-sanitization, TASK-stage-layout-repair, TASK-stage-numeric-symbolic-verbalization, TASK-stage-abbreviation-expansion, TASK-model-service | 2026-06-16 | Phase 5.1. Synchronous; verify latency bounds (≤10s/2MB, ≤1s/500-char). 409 if no model loaded. REQ-F-upload-text-file now satisfied here |
| TASK-synthesis-api-text-input | Change POST /jobs/synthesis from multipart file upload to JSON confirmed text; derive disk estimate from text length | P1 | Todo | [REQ-F-synthesize-audiobook](../1-objectives/requirements/REQ-F-synthesize-audiobook.md), [REQ-USA-normalized-text-review](../1-objectives/requirements/REQ-USA-normalized-text-review.md) | TASK-synthesis-job-api, TASK-preprocess-api | 2026-06-16 | Phase 5.1. Supersedes the multipart contract of TASK-synthesis-job-api; synthesizes exactly the confirmed text, no re-preprocessing |
...
| TASK-creation-view-review-step | Rework audiobook-creation view: upload → call /preprocess → review/confirm normalized text → synthesize confirmed text | P1 | Todo | [REQ-USA-normalized-text-review](../1-objectives/requirements/REQ-USA-normalized-text-review.md) | TASK-audiobook-creation-view, TASK-preprocess-api, TASK-synthesis-api-text-input | 2026-06-16 | Phase 5.1. Pass the same language to /preprocess and /jobs/synthesis; show a busy state during the synchronous preprocess call |
...
| TASK-phase-5.1-manual-testing | Update runbook and component docs for text preprocessing and the normalized-text review flow | P1 | Todo | - | TASK-creation-view-review-step | 2026-06-16 | Covers Phase 5.1. Include a representative messy PDF-extracted input to subjectively assess GOAL-text-normalization criterion 6 ("measurably more natural") |
