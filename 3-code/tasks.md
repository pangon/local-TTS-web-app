...
| TASK-voice-clone-script | Create an offline voice-clone-prompt generation script (audio + transcript → `wavs/qwen3-tts/<name>.pt`) for Qwen3-TTS Base | P2 | Done | [REQ-F-voice-language-selection](../1-objectives/requirements/REQ-F-voice-language-selection.md) | TASK-loader-qwen3-tts | 2026-06-22 | Phase 5.4 (first task). Per DEC-voice-clone-prompts. New standalone CLI `src/local_tts/scripts/generate_voice_clone_prompt.py` (run `python -m local_tts.scripts.generate_voice_clone_prompt <audio> --text "…"` / `--text-file PATH`, `--name` default `default`, `--output-dir` default `config.QWEN3_TTS_PROMPTS_DIR`, `--model` default `Qwen/Qwen3-TTS-12Hz-1.7B-Base`, `--device` auto). Loads the Base model (`Qwen3TTSModel.from_pretrained`, bf16+flash-attn on CUDA / float32 on CPU), calls `create_voice_clone_prompt(ref_audio, ref_text)` (ICL mode — transcript required) and `torch.save`s the `list[VoiceClonePromptItem]` to `<output-dir>/<name>.pt`. New `src/local_tts/scripts/__init__.py` documents the subpackage as non-runtime operator tools. New `config.QWEN3_TTS_PROMPTS_DIR` (env `LOCAL_TTS_QWEN3_TTS_PROMPTS_DIR`, default repo-root `wavs/qwen3-tts/`, gitignored under `/wavs/`) — shared artifact location reused by TASK-loader-qwen3-tts-base. Builder runs **only** in the script, never in the request path; clear errors on missing audio / empty transcript (no fallback). Self-documents usage (no manual-testing task this phase, user choice). A source-scan test enforces the script is not imported by `app.py`/`api/` (DEC required check #1). Backend component doc updated. 17 new tests (`test_generate_voice_clone_prompt_script.py`, qwen_tts mocked); full backend suite **941 passing** |
