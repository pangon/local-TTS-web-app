...
| TASK-model-license-metadata | Add license metadata (license, license_is_foss, license_notice) to COMPATIBLE_MODELS and expose it in GET /models | P2 | Todo | [REQ-F-model-listing](../1-objectives/requirements/REQ-F-model-listing.md) | TASK-model-service | 2026-06-20 | Phase 5.2. Per DEC-model-license-disclosure. Adds the three license fields to each COMPATIBLE_MODELS entry (populate existing models too, verified against their HF cards) and to the `GET /models` response. Precondition for the per-adapter license registration below and for TASK-model-license-notice-ui |
| TASK-loader-voxcpm2 | Implement VoxCPM2 adapter; register the model in COMPATIBLE_MODELS with loader_available + license metadata | P2 | Todo | [REQ-F-synthesize-audiobook](../1-objectives/requirements/REQ-F-synthesize-audiobook.md) | TASK-model-adapter-interface, TASK-model-license-metadata | 2026-06-20 | Phase 5.2. `openbmb/VoxCPM2`, **Apache-2.0 (FOSS)**. Loader: `voxcpm` pip package, `VoxCPM.from_pretrained()` (not AutoModel). 30 langs incl. Italian (auto-detected, no language tag); 48 kHz output (verify audio pipeline handles >24 kHz); ~2B params, ~8 GB VRAM. See architecture Compatibility Table |
...
| TASK-phase-5.1-manual-testing | Update runbook and component docs for text preprocessing and the normalized-text review flow | P1 | Done | - | TASK-creation-view-review-step | 2026-06-18 | Covers Phase 5.1. Added runbook §Phase 5.1 (12 procedures: /preprocess file/text/validation/409, the four stages, per-language/per-model config + domain dictionary, latency bound, creation-view review flow, exact-text guarantee, and a messy PDF-style sample for GOAL-text-normalization criterion 6) + preprocessing troubleshooting rows; updated test-coverage descriptions; added preprocess/synthesis API bullet (backend) and creation-view review-flow bullet (frontend) to component docs. Pre-existing: Phase 1-5 runbook curl/SSE refs use `/api/...` (real prefix is `/api/v1`, SSE `/api/v1/events`) — flagged, not swept |
