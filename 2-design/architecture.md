...
> **transformers baseline:** the backend pins **`transformers>=5.5`** (`DEC-transformers-5x-baseline`), required by MOSS-TTSD v1.0 (≥5.0) and Higgs Audio v3 (≥5.5). Consequently the Qwen3-TTS adapter is **unregistered** — its `qwen-tts` package hard-pins `transformers==4.57.3` (no 5.x-compatible release) — and any new model-loading library chosen here must be compatible with transformers 5.x. For models loaded via `trust_remote_code`, pin the repo `revision` so the downloaded model code cannot silently drift.
...
| `Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice` | No | `qwen-tts` pip package, `Qwen3TTSModel` | `model_type: qwen3_tts` not in standard transformers; needs custom tokenizer + speech tokenizer. ⚠️ **Adapter currently unregistered**: `qwen-tts` hard-pins `transformers==4.57.3`, incompatible with the transformers ≥5.5 baseline (`DEC-transformers-5x-baseline`). The adapter module is retained for re-registration if a 5.x-compatible `qwen-tts` ships |
