...
> **transformers baseline (currently EXPLORATORY / in flux — `DEC-transformers-5x-baseline`):** MOSS-TTSD v1.0 (≥5.0) and Higgs Audio v3 (≥5.5) need transformers **5.x**, whereas Fish Audio S2-Pro's `fish-speech` loader hard-pins **`transformers<=4.57.3`** (and `torch==2.8.0`) — a single environment cannot host both. The user put the baseline in an exploratory state (2026-06-21): the `pyproject` pin is widened to **`transformers>=4.38`** and the operator installs the major the model under trial needs (currently `4.57.3` for Fish S2-Pro). Under a 4.x install, MOSS-TTSD/Higgs v3 are not loadable at runtime (documented trade-off); under 5.x, Fish S2-Pro is not. The Qwen3-TTS adapter stays **unregistered** (its `qwen-tts` package can't be co-installed at all). For models loaded via `trust_remote_code`, prefer pinning the repo `revision` so the downloaded code cannot silently drift — **unless** the loader forwards `revision` into a different-repo sub-component (MOSS-TTSD propagates it to its companion codec, so that adapter must load at HEAD). To be re-fixed to a single stable baseline later.
