...
| `Qwen/Qwen3-TTS-12Hz-1.7B-Base` | No | `qwen-tts` pip package, `Qwen3TTSModel` — the **Base** variant: voice **cloning** via `generate_voice_clone(text, language, voice_clone_prompt=…)`, where the prompt is built once **offline** by `Qwen3TTSModel.create_voice_clone_prompt(ref_audio, ref_text)` (returns `list[VoiceClonePromptItem]`, torch-serializable) | **Apache-2.0 (FOSS).** Same `qwen-tts` package as the CustomVoice variant → same **transformers 4.57.3** baseline; GPU-host dependency, **adapter registered** (`loader_available=true`) only when installed at 4.57.3 (`DEC-transformers-5x-baseline`). **Translate-ISO** language (explicit language like the CustomVoice variant / Kokoro / XTTS — the adapter translates the ISO 639-1 code to the model's language name; not auto-detect). **Voice = a precomputed voice-clone prompt** loaded from disk (`DEC-voice-clone-prompts`): the offline cloning script writes `wavs/qwen3-tts/<voice-name>.pt`; the adapter loads the configured **default** prompt (`config.QWEN3_TTS_DEFAULT_VOICE_PROMPT_PATH` → `wavs/qwen3-tts/default.pt`) and passes it to `generate_voice_clone`. **No built-in speaker and no raw-clip fallback** — a missing prompt raises a clear error telling the operator to run the cloning script (unlike CosyVoice 3 / F5-TTS, which clone from a raw clip at runtime). Per-request prompt selection arrives with Phase 6 voice selection. 24 kHz |
...
### Voice Cloning via Precomputed Prompts

Some models clone a target voice from a **precomputed prompt artifact** the model produces from a reference clip plus its transcript, rather than from a raw clip at request time (`DEC-voice-clone-prompts`). The driving case is `Qwen/Qwen3-TTS-12Hz-1.7B-Base`, whose `qwen-tts` API builds a prompt with `create_voice_clone_prompt(ref_audio, ref_text)` (speaker embedding + reference codes, as torch tensors) and consumes it via `generate_voice_clone(text, language, voice_clone_prompt=…)`. Building the prompt requires running the model and a transcript, so it is done **once, offline**, not on every synthesis.

The mechanism is written generically (opt-in per adapter) but is currently used only by the Qwen3-TTS Base adapter:

- **Offline generation.** A standalone backend **script** (not part of the product runtime, **not** exposed via any API or the frontend) takes an audio file (e.g. `.mp3`) plus its transcript, builds the clone prompt, and serializes it with `torch.save`. It is a manual operator tool, run on the GPU host.
- **On-disk layout.** Prompt artifacts live under a **per-model-family** subfolder of the repo-root `wavs/` directory — `wavs/qwen3-tts/<voice-name>.pt`. Artifacts are model-specific and **not** portable across models. The `wavs/` tree is gitignored (user-provided assets).
- **Adapter consumption.** The adapter loads the configured **default** prompt (`config.QWEN3_TTS_DEFAULT_VOICE_PROMPT_PATH` → `wavs/qwen3-tts/default.pt`, env-overridable, read at call time) and passes the deserialized prompt to the model. A **missing** artifact raises a clear, actionable error (run the cloning script) — there is **no** silent fallback to raw-clip cloning or a built-in speaker.

This is distinct from the raw-clip stopgap used by CosyVoice 3 and F5-TTS, where the adapter is handed `config.DEFAULT_VOICE_PATH` (`wavs/default.mp3`) and clones at runtime each call. Precomputed-prompt models prepare a model-specific artifact ahead of time instead. Both mechanisms are stopgaps for the absence of in-product voice selection; per-request selection among the prepared prompts arrives with Phase 6 (`TASK-voice-language-selection-ui`).
