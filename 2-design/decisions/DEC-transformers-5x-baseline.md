...
- **Currently installed: `transformers==5.12.1` + `huggingface-hub==1.20.1`** (flipped back up 2026-06-21, at the user's request, to exercise Higgs Audio v3 / MOSS-TTSD — the inverse of the 2026-06-21 downgrade to `4.57.3` made for Fish S2-Pro). The full backend suite (812 tests) passes on both 4.57.3 and 5.12.1; `torch` stays at 2.10.0.
- **Runtime trade-off (direction-dependent):** under the **current 5.x install**, **Fish Audio S2-Pro cannot load at runtime** (its `fish-speech` engine needs `transformers<=4.57.3` / `torch==2.8.0`, and it is GPU-host-only anyway); under a **4.x install**, **MOSS-TTSD v1.0 and Higgs Audio v3 cannot load at runtime** (they need ≥5.0/≥5.5). Either way, all three adapters remain *registered* (`loader_available=true`) because the adapter modules import fine on both majors and their unit tests are mocked — the incompatibility is a documented, reversible runtime consequence of the exploratory baseline, not an install conflict. (Contrast Qwen3-TTS, which is *unregistered* because its `qwen-tts` package cannot even be co-installed.)
