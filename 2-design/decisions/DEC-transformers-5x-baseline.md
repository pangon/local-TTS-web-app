...
- **`pyproject.toml` widens the pin to `transformers>=4.38`** (no upper bound) so the operator can install whichever major the model under exploration needs. The operator installs the latest 5.x to exercise MOSS-TTSD/Higgs/VoxCPM2/Kokoro, or downgrades to `4.57.3` to exercise Qwen3-TTS / Fish S2-Pro.
  - **Note (2026-06-21):** the `coqui-tts` core dependency (XTTS-v2, `TASK-loader-xtts-v2`) requires `transformers>=4.57` (a floor, **no** upper bound — not a hard `==`/`<=` pin), so the *effective* floor for a full `pip install -e .` is now 4.57. This does **not** affect the exploratory 4.57.3 ⇄ 5.x flip (both satisfy `>=4.57`) and nothing in the catalog needs `<4.57`. Because it is a floor (not a hard pin) and is compatible at both baselines, `coqui-tts` is permitted as a normal runtime dependency (like kokoro/voxcpm) — only `==`/`<=` hard-pinning packages (qwen-tts `==4.57.3`, fish-speech `<=4.57.3`+`torch==2.8.0`) are excluded.
...
- **CosyVoice 3 is a third GPU-host dependency with its own distinct pin (2026-06-21):** the `cosyvoice` package (CosyVoice 3, `TASK-loader-cosyvoice`) hard-pins `transformers==4.51.3`, `torch==2.3.1` and `numpy==1.26.4` (verified against upstream `requirements.txt`) — distinct from both qwen-tts/fish-speech (`<=4.57.3`) and MOSS-TTSD (`>=5.x`), so no single shared environment satisfies it either. It is therefore a **GPU-host dependency** (installed from the CosyVoice repo in a dedicated environment), **not** a backend runtime dependency; its `CosyVoice3Adapter` lazy-imports the package (clear install hint), unit tests mock it, and the adapter is **registered** uniformly (loadability is a runtime function of the installed environment). This reinforces, rather than changes, the decision: hard-pinning packages stay GPU-host-only.
- **F5-TTS is a fourth GPU-host dependency (2026-06-21):** the `f5-tts` package (F5-TTS, `TASK-loader-f5-tts`) pins `numpy<=1.26.4` on Python ≤3.10 (verified against upstream `pyproject.toml`), conflicting with the baseline's `numpy` 2.x (required by `coqui-tts` / `voxcpm`), and pulls a heavy dependency tree (gradio, wandb, bitsandbytes, hydra-core, vocos, x_transformers, torchcodec). Although it does not hard-pin `transformers`/`torch` (`transformers` unconstrained, `torch>=2.0.0`), the `numpy` conflict + heavy footprint make it unfit as a backend runtime dependency, so it is a **GPU-host dependency** (`pip install f5-tts` in a dedicated environment), **not** in `pyproject.toml`; its `F5TTSAdapter` lazy-imports the package (clear install hint), unit tests mock it, and the adapter is **registered** uniformly (loadability is a runtime function of the installed environment). Same treatment as cosyvoice/qwen-tts/fish-speech: incompatible-with-the-baseline packages stay GPU-host-only.
