...
| 2026-06-20 | Extended the sentence-segmentation stage with **dialogue isolation**: a spoken span (`«` … `»`) is put on its own line(s) by breaking before `«` and after `»`, so the narration, the quote, and any trailing dialogue tag are distinct chunks. Surfaced on the same book file: the close of a quote was glued to the following tag/narration, and the `»`→`"` flattening discarded the line break. The Unicode stage no longer flattens the directional guillemets (it still flattens all other quote variants); the segmentation stage flattens them to `"` after using their direction. User approved breaking at the start of the quote too (symmetric isolation). | human-decided |
