...
| 2026-06-21 | **F5-TTS added as a fourth GPU-host dependency** (`TASK-loader-f5-tts`, Phase 5.3 — final task). The `f5-tts` package pins `numpy<=1.26.4` on Python ≤3.10 (conflicts with the baseline's numpy 2.x) and pulls a heavy dep tree; though it doesn't hard-pin `transformers`/`torch`, that conflict + footprint make it GPU-host-only (lazy-imported, mocked in tests), not a runtime dep; the `F5TTSAdapter` is registered uniformly (`loader_available=true`). Reinforces (does not change) the decision. Full suite 924 green on 4.57.3 | ai-proposed/human-approved |
