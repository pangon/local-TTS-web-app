...
5. **Sentence segmentation** — put each sentence on its own line so the reviewed text is segmented into the sentences the author intended and mirrors the synthesizer's own sentence chunking (`REQ-F-text-layout-repair`, `REQ-USA-normalized-text-review`). This runs **last**, after numeric/symbolic verbalization and abbreviation expansion, so a sentence-ending period is no longer ambiguous with a thousands separator (`11.988`) or an abbreviation dot (`E.F.`, `sig.`). It splits — the inverse of layout repair's reflow, which rejoins soft-wrapped fragments — while preserving blank-line paragraph boundaries and already-standalone structural lines (headings, list items, bare numbers). It also performs **dialogue isolation**: a spoken span delimited by directional double-angle guillemets (`«` … `»`) is put on its own line(s) by breaking before an opening `«` and after a closing `»`, so the narration before a quote, the quote itself, and any trailing dialogue tag (`… all'atterraggio» comunicò.`) each become distinct chunks. To make this possible the Unicode-sanitization stage leaves `«`/`»` intact (it flattens every other quote variant); this stage flattens them to straight `"` after using their direction.
