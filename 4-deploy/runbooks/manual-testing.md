...
Phase 3.1 introduces the model adapter layer: each model in the compatible list is annotated with a `loader_available` flag indicating whether a concrete adapter is registered for it. Models **without** an adapter cannot be downloaded or loaded and are surfaced as such in the UI. Adapters currently registered: **Kokoro-82M** (`hexgrad/Kokoro-82M`), **VoxCPM2** (`openbmb/VoxCPM2`), **MOSS-TTSD v1.0** (`OpenMOSS-Team/MOSS-TTSD-v1.0`), **Fish Audio S2-Pro** (`fishaudio/s2-pro`), and **Qwen3-TTS** (`Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice`). Note these adapters' **runtime loadability depends on the installed transformers major** under the exploratory baseline (`DEC-transformers-5x-baseline`): Qwen3-TTS and Fish S2-Pro need **transformers 4.57.3** (the version `qwen-tts` / `fish-speech` require), while MOSS-TTSD needs **≥5.0**; the currently installed major is **4.57.3**, so MOSS-TTSD is not runtime-loadable in this state (and vice-versa under a 5.x install). All five report `loader_available=true` regardless; only Higgs v3 stays unregistered (server-only).
...
4. Verify `loader_available` is `true` for `hexgrad/Kokoro-82M`, `openbmb/VoxCPM2`, `OpenMOSS-Team/MOSS-TTSD-v1.0`, `fishaudio/s2-pro`, and `Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice`, and `false` for models without a registered adapter (e.g. `coqui/XTTS-v2`, `ResembleAI/chatterbox`, and `bosonai/higgs-audio-v3-tts-4b` — server-only, per `DEC-transformers-5x-baseline`).
...
> **MOSS-TTSD v1.0** (`OpenMOSS-Team/MOSS-TTSD-v1.0`) is also registered (Apache-2.0, ~19 GB VRAM, transformers ≥5.0). It is a dialogue/multi-speaker model used here for single-voice narration (each input is wrapped as a single `[S1]` turn); validate it on a GPU with sufficient VRAM **and a transformers 5.x install**. The **Qwen3-TTS** adapter (`Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice`) is registered and loads on a **transformers 4.57.3** install (the version its `qwen-tts` package requires) — verified end-to-end (load + 24 kHz Italian generation). Per the exploratory baseline (`DEC-transformers-5x-baseline`), MOSS-TTSD (≥5.x) and Qwen3-TTS/Fish S2-Pro (4.57.3) are not loadable at the same time; flip the installed `transformers` major to match the model under test.
...
| Model shows "No adapter" badge with no Download/Load buttons | Expected: no adapter is registered for that model. Use an adapter-backed model (Kokoro-82M, VoxCPM2, MOSS-TTSD, Fish S2-Pro, or Qwen3-TTS), or implement the corresponding `TASK-loader-*` adapter. Note Higgs v3 is intentionally unregistered (server-only, `DEC-transformers-5x-baseline`) |
| MOSS-TTSD load fails with `cannot import name 'PreTrainedConfig'` | MOSS-TTSD needs `transformers>=5.0`; this error means an older transformers (e.g. the 4.57.3 used for Qwen3-TTS / Fish S2-Pro) is installed. The baseline is exploratory (`DEC-transformers-5x-baseline`) — install transformers 5.x (`pip install "transformers>=5.5"`) to exercise MOSS-TTSD, accepting that Qwen3-TTS / Fish S2-Pro then become non-loadable |
| Qwen3-TTS load fails (e.g. `KeyError: 'default'` in RoPE init, or `config ... has no attribute 'pad_token_id'`) | `qwen-tts` requires **transformers 4.57.3** and does not run on transformers 5.x; the installed major is 5.x. Per the exploratory baseline (`DEC-transformers-5x-baseline`) install `pip install "transformers==4.57.3"` (pulls huggingface-hub 0.36.x; accelerate 1.12.0) and `pip install qwen-tts`, accepting that MOSS-TTSD then becomes non-loadable |
| `Insufficient VRAM to load <model>. Required: X MB, Available: Y MB` | The preflight estimates VRAM as the model's on-disk size × an overhead factor (default 1.5, `REQ-F-gpu-validation`). For a model that is borderline on a smaller GPU, lower the factor via `LOCAL_TTS_VRAM_OVERHEAD_FACTOR` (e.g. `LOCAL_TTS_VRAM_OVERHEAD_FACTOR=1.1`) and restart the backend — but a value too low risks a real out-of-memory during synthesis (especially on long chapters); for reliable use prefer a GPU with more VRAM or a smaller model (VoxCPM2 ~8 GB, Kokoro). Also free other GPU processes (`nvidia-smi`) and optionally set `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` to reduce fragmentation |
...
| F5-TTS synthesis fails with `Could not load libtorchcodec` / `libnvrtc.so.13: cannot open shared object file` | F5-TTS auto-transcribes the reference clip via a transformers Whisper pipeline that imports `torchcodec`, and the transitively-installed `torchcodec` (0.14, built for CUDA 13) is incompatible with torch 2.10+cu128 (bundles `libnvrtc.so.12`). Install the matching build: `pip install "torchcodec==0.10.*" --index-url https://download.pytorch.org/whl/cu128` (torch 2.10 ↔ torchcodec 0.10; match the `cuXXX` index to your torch). Model **load** doesn't need torchcodec — only reference-clip auto-transcription does (verified 2026-06-21 on RTX 4070 Ti). Note `pip install f5-tts` also downgrades numpy 2.x→1.26.4, which is harmless (voxcpm/coqui-tts/kokoro still import) |
